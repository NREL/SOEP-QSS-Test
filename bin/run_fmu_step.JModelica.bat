...
if exist "%model_dir%%model%.var" (
  call jm.JModelica.bat run_fmu_step.py %tool_dir%%model%.fmu --var %model_dir%%model%.var %*
) else (
if exist "%tool_dir%%model%.fmu" (
  call jm.JModelica.bat run_fmu_step.py %tool_dir%%model%.fmu %*
) else (
  call jm.JModelica.bat run_fmu_step.py %*
))