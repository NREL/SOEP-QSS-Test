...
#   print( 'TwoFloor_TwoZone' ); subprocess.run( 'cd TwoFloor_TwoZone && cmp_PyFMI_QSS.py --bin --zrFac=10000 --dtND=1e-4 --dtInf=0.001 --cmp=weaDat.weaBus.TDryBul --cmp=weaDat.weaBus.HDirNor --cmp=weaDat.weaBus.HGloHor --cmp=buiZon.theZon[1,1].roo.heaPorAir.T --cmp=buiZon.theZon[2,1].roo.heaPorAir.T --cmp=buiZon.theZon[1,1].heaCooPow --cmp=buiZon.theZon[2,1].heaCooPow', stdout = log, stderr = subprocess.STDOUT, shell = True ) # Event indicator issues cause the QSS runs to stall
