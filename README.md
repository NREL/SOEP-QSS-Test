...
* `bld_fmu.*.py`  Builds model `.fmu` with the specified Tool or options
* `cleanup`  Removes comparison/regression testing output files
* `cmp_CVode_QSS_*.py`  Run and compare CVode and QSS simulations for a set of models (wraps `cmp_PyFMI_QSS.py`)
...
* `mod_xml.py`  Adds index comments to an FMU's modelDescription.xml file (not needed for OCT or recent JModelica)
* `ref.py`  Runs the model's `.fmu` with PyFMI or QSS depending on the local directory: small tolerance "reference" solution
* `ref.#[shdy].py`  Wraps `ref.py` with a specified end time
...
* `run.#[shdy].py`  Wraps `run.py` with a specified end time
...
* `set_OCT_QSS*`  Sets environment for OCT + QSS: Customize to your system
...
### `mdl` Notes
* Custom versions of the .mo Modelica file for some models with variables for zero-crossing functions added is in the JModelica subdirectory.
  These versions have explicit \_\_zc\_*name* and \_\_zc\_der\_*name* variables added for the zero-crossing functions and their derivatives that QSS can use.
...
Tool/
  ...
  modelDescription.prep.xml     Modified FMU XML
  modelDescription.xml          XML in the FMU (after any modifications are made)
  out/                          Results
...
The QSS2 method is currently the best choice in most circumstances (QSS3 performance and accuracy are limited pending automatic differentiation support in FMUs) so the other sub-directories may not be present.
...
* The under-development event indicator support in OCT should generate FMUs from models with if/when conditional blocks (almost all models) that QSS can correctly and efficiently simulate.
  Other Modelica tool FMU generators lack this support and are not practical for QSS use.
* The FMI/FMIL APIs do not provide a mechanism to tell the FMU to process a conditional event at a specific time, so QSS advances the relevant variables a small time step beyond the potential conditional event in the hope that the FMU will then detect and process the event if an event actually occurs at that time. This may not yet be robust for all models, especially due to the current dependence on numerical differentiation.
  Until a better mechanism is available it is possible for QSS solutions to miss some conditional events.
