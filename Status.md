...
2. Some FMU builds give warnings like this (may be a code gen issue or a problem in the Buildings Library file):
   ```
   ...
   ```
3. Some PyFMI CVode runs give extra output points near startup like:
   ```
   0	1
   ...
   * These appear to be generated by actual zero crossing events at the start time that PyFMI solvers would normally pick up after the start time but that QSS will not currently handle ([#14](https://github.com/NREL/SOEP-QSS-Test/issues/14)).
4. Getting the PyFMI results variable list using the prior `res.keys()` method stopped working. The `run_PyFMI.py` script uses this list for output filtering against a name/wildcard/regex list of variables. A work-around was found (getting it from `res._result_data.vars`) so it isn't an obstacle.
5. The [TwoFloor_TwoZone](https://github.com/NREL/SOEP-QSS-Test/tree/main/mdl/TwoFloor_TwoZone) CVode run now aborts if the FMU is built with generate_ode_jacobian but during simulation, not initialization.
6. Repeat value references were found in the FMUs built with this OCT update in a number of models ([#15](https://github.com/NREL/SOEP-QSS-Test/issues/15)).
